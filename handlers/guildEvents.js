...
export function registerGuildEvents(client, shardId) {
  ...
  client.on(Events.MessageCreate, async (message) => {
    try {
      ...
      const lastMsg = messageCooldowns.get(message.author.id);
      ...
      db.prepare('UPDATE users SET total_messages = total_messages + 1 WHERE user_id = ?')
        .run(message.author.id);
  ...
  const bumpReputation = async (reaction, user, delta) => {
    try {
      ...
      getDb().prepare(
        'UPDATE users SET total_reactions_received = MAX(0, COALESCE(total_reactions_received, 0) + ?) WHERE user_id = ?',
      ).run(delta, message.author.id);
    } catch (err) {
...
export function startVoiceFarmLoop() {
  ...
  setInterval(async () => {
    try {
      ...
      for (const [, farmers] of voiceFarming) {
        for (const [userId, session] of farmers) {
          try {
            ...
            const user = db.prepare('SELECT last_voice_reset_time FROM users WHERE user_id = ?').get(userId);
            ...
            if (!member.voice?.selfDeaf && !member.voice?.selfMute) {
              db.prepare("UPDATE users SET last_voice_reset_time = datetime('now') WHERE user_id = ?").run(userId);
            }

            addCoins(userId, effectiveRate);
            const xpResult = addXp(userId, effectiveRate);
            db.prepare("UPDATE users SET last_voice_farm = datetime('now'), total_voice_minutes = total_voice_minutes + 1 WHERE user_id = ?").run(userId);
