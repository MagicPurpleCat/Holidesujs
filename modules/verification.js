...
export async function handleVerificationButton(interaction) {
  ...
  const user = db.prepare('SELECT is_verified FROM users WHERE user_id = ?').get(userId);
...
// Выдаём роль "Верифицирован"
    ...
    const userData = db.prepare('SELECT level FROM users WHERE user_id = ?').get(userId);
    if (userData && member) {
