 ...
 */
function collectProfileData(target, member, db) {
...
// Получаем партнера по браку
  ...
  if (user.relationship_status === 'married' && user.relationship_partner_id) {
    const partner = db.prepare('SELECT user_id FROM users WHERE user_id = ?').get(user.relationship_partner_id);
    if (partner) {
  ...
  try {
    const users = db.prepare(`
      ...
      FROM users WHERE is_infinite_balance = 0
    `).all();
...
export default {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const profileData = collectProfileData(target, member, db);
...
export async function handleProfileModals(interaction) {
  ...
  try {
    ...
    ensureUser(userId);
    db.prepare('UPDATE users SET personal_note = ?, status_text = ? WHERE user_id = ?')
      .run(about, status, userId);
...
export async function handleProfileSelectMenus(interaction) {
  ...
  try {
    ...
    db.prepare('UPDATE users SET gender = ? WHERE user_id = ?').run(genderValue, userId);
