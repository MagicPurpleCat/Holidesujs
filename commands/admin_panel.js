...
export async function handleAdminPanelButtons(interaction) {
  ...
  if (customId === 'admin_stats' || customId === 'ap_server_stats') {
    ...
    const totalBalance = db.prepare(
      'SELECT SUM(balance) as total FROM users WHERE is_infinite_balance = 0'
    ).get().total || 0;
    const topLevel = db.prepare(
      'SELECT MAX(level) as max FROM users'
    ).get().max || 1;
...
export async function handleGrantModal(interaction) {
  ...
  db.prepare(
    'INSERT OR IGNORE INTO users (user_id, balance, xp, level) VALUES (?, 100, 0, 1)'
  ).run(targetId);
...
export async function handleUnverifyModal(interaction) {
  ...
  db.prepare('UPDATE users SET is_verified = 0 WHERE user_id = ?').run(targetId);
...
export async function handleGiveVerifyModal(interaction) {
  ...
  const user = db.prepare('SELECT is_verified, level FROM users WHERE user_id = ?').get(targetId);
  if (user?.is_verified) {
    return interaction.reply({
  ...
  db.prepare('UPDATE users SET is_verified = 1 WHERE user_id = ?').run(targetId);
  ...
  // Выдаём роль уровня
  const userData = db.prepare('SELECT level FROM users WHERE user_id = ?').get(targetId);
  let levelRoleMsg = '';
  if (member && userData) {
...
export async function handleEconomyModal(interaction) {
  ...
  if (customId === 'ap_clear_infinite_modal') {
    db.prepare('UPDATE users SET is_infinite_balance = 0 WHERE user_id = ?').run(targetId);
  ...
  switch (customId) {
    ...
    case 'ap_remove_balance_modal': {
      const user = db.prepare('SELECT balance FROM users WHERE user_id = ?').get(targetId);
    ...
    case 'ap_set_infinite_modal': {
      db.prepare(
        'UPDATE users SET balance = ?, is_infinite_balance = 1 WHERE user_id = ?'
      ).run(amount, targetId);
      embed = new EmbedBuilder()
        .setColor(0x9b59b6)
    ...
    case 'ap_add_xp_modal': {
      const xpResult = addXp(targetId, amount);
      let levelUpMsg = '';
      if (xpResult && member) {
    ...
    case 'ap_remove_xp_modal': {
      const userBefore = db.prepare('SELECT level FROM users WHERE user_id = ?').get(targetId);
      const oldLevel = userBefore.level;
      const newLevel = removeXp(targetId, amount);
      let levelDownMsg = '';
...
export async function handleModerationModal(interaction) {
  ...
  if (customId === 'ap_warns_modal') {
    const punishments = db.prepare(`
      SELECT * FROM punishments WHERE user_id = ? AND action = 'warn'
