...
async function handleBuyRole(interaction, roleDbId) {
  ...
  try {
    ...
    if (!discordRole) {
      runInTransaction(() => {
        addCoins(user.id, role.price);
        db.prepare(
          'UPDATE custom_roles SET current_holders = MAX(current_holders - 1, 0) WHERE id = ?'
        ).run(roleDbId);
        if (creatorProfit > 0) removeCoins(role.creator_id, creatorProfit);
      });
      return interaction.reply({
...
async function handleBuyBoost(interaction, itemId) {
  ...
  try {
    runInTransaction(() => {
      if (!removeCoins(user.id, item.price)) {
        throw new Error('NO_FUNDS');
      }
      db.prepare(`
...
async function startCreationFlow(interaction) {
  ...
  // Проверка баланса
  const user = db.prepare('SELECT balance FROM users WHERE user_id = ?').get(interaction.user.id);
...
async function handleCreationConfirm(interaction) {
  ...
  try {
    runInTransaction(() => {
      if (!removeCoins(interaction.user.id, CREATION_COST)) {
        throw new Error('NO_FUNDS');
      }
      db.prepare(
